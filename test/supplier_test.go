...
var _ = Describe("Supplier API", func() {
	Describe("Create and Get Supplier", func() {
		...
		It("should create and get supplier with admin role", func() {
			...
			// Create supplier
			resp, err := client.MakeRequest("POST", "/api/v1/suppliers", supplierData, WithAuth())
			Expect(err).NotTo(HaveOccurred())
			Expect(resp.StatusCode).To(Equal(201))

			supplierResp := ParseResponse(resp)
			Expect(supplierResp["id"]).NotTo(BeNil())
			Expect(supplierResp["name"]).To(Equal("Test Supplier"))

			supplierID := ToString(supplierResp["id"])

			// Get the supplier
			resp, err = client.MakeRequest("GET", "/api/v1/suppliers/"+supplierID, nil, WithAuth())
			Expect(err).NotTo(HaveOccurred())
			Expect(resp.StatusCode).To(Equal(200))

			getSupplierResp := ParseResponse(resp)
	...
	Describe("Update Supplier", func() {
		...
		Context("when user has authorized role", func() {
			It("should update supplier with admin role", func() {
				...
				resp, err := client.MakeRequest("PUT", urlPath, updatedSupplierData, WithAuth())
				Expect(err).NotTo(HaveOccurred())
				Expect(resp.StatusCode).To(Equal(200))

				updatedSupplierResp := ParseResponse(resp)
