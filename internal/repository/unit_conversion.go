...
func (r *unitRepository) CreateConversion(ctx context.Context, conversion *models.UnitConversion) error {
	return r.txnCreateConversion(r.db.WithContext(ctx), conversion)
}

func (r *unitRepository) txnCreateConversion(db *gorm.DB, conversion *models.UnitConversion) error {
	// flip the from and to unit IDs in order to keep the conversion factor positive.
	...
	err := db.Create(conversion).Error
	if err != nil {
		if isDuplicateError(err, nil) {
			return pkg.ErrUnitConversionAlreadyExists(db.Statement.Context, conversion.FromUnitID, conversion.ToUnitID)
		}
		return err
	}
	return nil
