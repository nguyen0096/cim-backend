...
func computeDisposePlan(ctx context.Context, db *gorm.DB, inventoryID, submissionID uint) (*DisposePlan, error) {
	...
	for _, id := range itemIDs {
		...
		for _, src := range item.ConsumableTransactions {
			...
			// txn's CreatedAt is backdated to sub.CreatedAt; if this source layer was
			// created AFTER the submission, the backdated disposal would predate its own
			// source stock and corrupt historical COGS. This is what an opening-stock
			// load stamped today looks like to a backdated dispose. Abort the whole plan
			// (no writes). Only post-dated sources FIFO actually reaches are checked: a
			// later layer the walk never consumes is skipped by the toConsume>0 guard.
			if src.CreatedAt.After(sub.CreatedAt) {
				return nil, fmt.Errorf(
					"item %d: FIFO would consume stock layer txn %d (created_at %s), which is AFTER the dispose submission's created_at %s; backdating the disposal to the submission date would place it before its source stock existed. Aborting (no writes). Approve this dispose via the normal app flow, or with a date on/after that layer.",
